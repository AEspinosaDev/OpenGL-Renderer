...
void InstancedMesh::draw() {
	...
	{
		m_Vaos[i].bind();
		GLcall(glDrawElementsInstanced(GL_TRIANGLES, m_BufferData[i].triangles, GL_UNSIGNED_INT, (void*)0, m_InstanceCount));
	}

}

void InstancedMesh::draw(int meshIdx)
{
	m_Vaos[meshIdx].bind();
	GLcall(glDrawElementsInstanced(GL_TRIANGLES, m_BufferData[meshIdx].triangles, GL_UNSIGNED_INT, (void*)0, m_InstanceCount));
}
